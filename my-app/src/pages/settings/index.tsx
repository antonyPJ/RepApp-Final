import React from "react";
import { View, Text, ScrollView, TouchableOpacity } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";
import { MaterialIcons, FontAwesome } from "@expo/vector-icons";

export default function Settings() {
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: "#F5F7FA" }}>
      <ScrollView 
        style={{ flex: 1 }} 
        contentContainerStyle={{ paddingBottom: 24 }}
      >
        
        {/* Header */}
        <View
          style={{
            backgroundColor: "#fff",
            padding: 16,
            borderBottomWidth: 1,
            borderColor: "#DCE3ED",
          }}
        >
          <Text style={{ color: "#4A90E2", fontSize: 14, marginBottom: 8 }}>
            Configurações de Acesso
          </Text>

          <Text
            style={{ fontSize: 22, fontWeight: "bold", color: "#2C2C2C" }}
          >
            Permissões e Funções
          </Text>

          <Text style={{ fontSize: 14, color: "#444", marginTop: 4 }}>
            Defina o que cada tipo de usuário pode visualizar e gerenciar no app
          </Text>
        </View>

        {/* Tabela */}
        <View
          style={{
            backgroundColor: "#fff",
            margin: 16,
            borderRadius: 12,
            padding: 16,
            borderWidth: 1,
            borderColor: "#D3D3D3",
          }}
        >
          {/* Header da tabela */}
          <View
            style={{
              flexDirection: "row",
              paddingBottom: 12,
              borderBottomWidth: 1,
              borderColor: "#CCC",
            }}
          >
            <Text style={{ flex: 1, fontWeight: "bold" }}>
              Funções realizáveis no App
            </Text>

            <View style={{ width: 90, alignItems: "center" }}>
              <MaterialIcons name="verified-user" size={22} color="#1A73E8" />
              <Text>Administrador</Text>
            </View>

            <View style={{ width: 90, alignItems: "center" }}>
              <FontAwesome name="user" size={22} color="#000" />
              <Text>Membro</Text>
            </View>
          </View>

          {/* Linhas da tabela */}
          {[
            "Criar/Participar de Eventos",
            "Visualizar Tarefas/Despesas",
            "Enviar mensagens/comentários",
            "Cadastrar/Editar Membros da República",
            "Editar/Excluir a República",
            "Gerenciar Finanças Gerais",
            "Gerenciar Permissões",
            "Aprovar/Excluir Eventos Criados por Outros",
            "Editar Dados Principais da República",
          ].map((item, index) => {
            const admin = true;
            const member = index < 3 ? true : false;

            return (
              <View
                key={index}
                style={{
                  flexDirection: "row",
                  paddingVertical: 12,
                  borderBottomWidth: index === 8 ? 0 : 1,
                  borderColor: "#EEE",
                }}
              >
                <Text style={{ flex: 1 }}>{item}</Text>

                <View style={{ width: 90, alignItems: "center" }}>
                  <MaterialIcons
                    name={admin ? "check-circle" : "cancel"}
                    size={22}
                    color={admin ? "#4CAF50" : "#E53935"}
                  />
                </View>

                <View style={{ width: 90, alignItems: "center" }}>
                  <MaterialIcons
                    name={member ? "check-circle" : "cancel"}
                    size={22}
                    color={member ? "#4CAF50" : "#E53935"}
                  />
                </View>
              </View>
            );
          })}
        </View>

        {/* Botões */}
        <View
          style={{
            flexDirection: "row",
            justifyContent: "space-between",
            paddingHorizontal: 16,
            paddingBottom: 20, // evita tocar a barra inferior
            paddingTop: 8,
          }}
        >
          <TouchableOpacity
            style={{
              backgroundColor: "#E5E5E5",
              padding: 14,
              flex: 1,
              borderRadius: 8,
              marginRight: 12,
              alignItems: "center",
            }}
          >
            <Text style={{ fontWeight: "600" }}>Cancelar</Text>
          </TouchableOpacity>

          <TouchableOpacity
            style={{
              backgroundColor: "#1A73E8",
              padding: 14,
              flex: 1,
              borderRadius: 8,
              alignItems: "center",
            }}
          >
            <Text style={{ fontWeight: "600", color: "#fff" }}>
              Salvar Alterações
            </Text>
          </TouchableOpacity>
        </View>

      </ScrollView>
    </SafeAreaView>
  );
}
